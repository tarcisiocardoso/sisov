...
public interface AnimalFotoRepository extends JpaRepository<AnimalFoto, Long>{
	...
	@Query("SELECT f.pathImagem FROM Foto f, AnimalFoto af WHERE af.idFoto = f.id AND af.idAnimal = ? AND f.dtInsert = ( select max(f2.dtInsert) from Foto f2 where f2.id = f.id)")
